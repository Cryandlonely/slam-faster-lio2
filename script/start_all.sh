...
# ============================================================
#  室外SLAM导航 一键启动脚本
#  包含: livox / vru_sensor / chassis / nav_planner(含RTK) / bridge
#  用法:
#    ./start_all.sh          # 启动全部模块
#    ./start_all.sh stop     # 停止全部模块
# ============================================================
...
_do_start() {
    ...
    bash -c "source /opt/ros/humble/setup.bash && source $SETUP_FILE && $cmd" >> "$logfile" 2>&1 &
    MODULE_PIDS["$name"]=$!
...
if [[ ! -f "$SETUP_FILE" ]]; then
    ...
    echo "请先执行: cd $WS_DIR && colcon build"
    exit 1
fi
...
launch_module "lidar_driver" "ros2 launch rslidar_sdk start.py"

# ---- 2. IMU/VRU 驱动 ----
echo -e "${YELLOW}[2/5] IMU/VRU 驱动${NC}"
launch_module "vru_sensor" "ros2 launch vru_sensor vru.launch.py" 1
...
launch_module "nav_planner" "ros2 launch nav_planner slam_nav_debug.launch.py" 2
...
echo -e "  停止全部: ${CYAN}./start_all.sh stop${NC}"
echo ""
